...
export function Sidebar() {
  ...
  return (
    <div className="w-64 bg-sidebar flex flex-col h-screen fixed left-0 top-0">
      <div className="p-6">
      ...
      <nav className="space-y-1 px-3">
        {navItems.map((item) => (
          <NavLink
            key={item.path}
            to={item.path}
            className="flex items-center gap-4 px-3 py-2 rounded-md text-muted-foreground hover:text-foreground transition-colors"
